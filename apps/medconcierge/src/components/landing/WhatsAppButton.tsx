...
export default function WhatsAppButton({ whatsappLink }: Props) {
  const [hovered, setHovered] = useState(false);
  return (
    <div className="fixed bottom-6 right-6 z-50">
      <a href={whatsappLink} target="_blank" rel="noopener noreferrer" onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)} className="group flex items-center gap-2 bg-[#25D366] hover:bg-[#20BD5A] text-white rounded-full shadow-xl shadow-green-900/30 transition-all duration-300 hover:shadow-2xl hover:shadow-green-900/40">
