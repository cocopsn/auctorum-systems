...
export default function SettingsClient({ tenantSlug, tenantName, logoUrl, config }: Props) {
  ...
  const inputClass =
    'w-full px-3 py-2 border border-gray-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#1B3A5C] transition';
    ...
    <form onSubmit={handleSubmit}>
      <Section title="Información general">
        ...
        <Field label="URL del logotipo">
          <input type="url" value={logo} onChange={e => setLogo(e.target.value)}
            placeholder="https://storage.ejemplo.com/logo.png" className={inputClass} />
        </Field>
      </Section>
          ...
          <Field label="Color primario">
            <div className="flex gap-2 items-center">
              ...
              <input type="text" value={primaryColor} onChange={e => setPrimaryColor(e.target.value)}
                placeholder="#1B3A5C" className={inputClass} />
            </div>
          </Field>
          <Field label="Color secundario">
            <div className="flex gap-2 items-center">
              ...
              <input type="text" value={secondaryColor} onChange={e => setSecondaryColor(e.target.value)}
                placeholder="#C0392B" className={inputClass} />
            </div>
          </Field>
        </div>
      </Section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Teléfono">
            <input type="tel" value={phone} onChange={e => setPhone(e.target.value)}
              placeholder="844 416 2555" className={inputClass} />
          </Field>
          <Field label="Correo electrónico">
            <input type="email" value={email} onChange={e => setEmail(e.target.value)}
              placeholder="ventas@empresa.com" className={inputClass} />
          </Field>
          <Field label="WhatsApp (con código de país)">
            <input type="text" value={whatsapp} onChange={e => setWhatsapp(e.target.value)}
              placeholder="528441234567" className={inputClass} />
          </Field>
          <Field label="Dirección">
            <input type="text" value={address} onChange={e => setAddress(e.target.value)}
              placeholder="Parque Industrial X, Nave 12, Saltillo" className={inputClass} />
          </Field>
        </div>
      </Section>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Field label="Razón social">
            <input type="text" value={razonSocial} onChange={e => setRazonSocial(e.target.value)}
              placeholder="Empresa S.A. de C.V." className={inputClass} />
          </Field>
          <Field label="RFC">
            <input type="text" value={rfc} onChange={e => setRfc(e.target.value)}
              placeholder="EMP123456ABC" className={`${inputClass} uppercase`} />
          </Field>
          <Field label="Giro comercial">
            <input type="text" value={giro} onChange={e => setGiro(e.target.value)}
              placeholder="Maquinados CNC de precisión" className={inputClass} />
          </Field>
        </div>
      </Section>
      ...
      <div className="flex justify-end">
        <button
          type="submit"
          disabled={loading}
          className="px-6 py-2.5 bg-[#1B3A5C] text-white font-medium text-sm rounded-lg hover:bg-[#15304d] disabled:opacity-50 transition-colors"
        >
