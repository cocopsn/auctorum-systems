...
// ---------------------------------------------------------------------------
// POST /api/dashboard/settings/security/2fa/verify
// ---------------------------------------------------------------------------
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!row?.two_factor_secret) {
      return NextResponse.json(
        { error: '2FA no ha sido iniciado. Activa primero.' },
        { status: 400 }
      );
    }
    ...
    await db.execute(
      sql`UPDATE users SET two_factor_enabled = true, two_factor_verified_at = NOW(), updated_at = NOW() WHERE id = ${auth.user.id}`
    );

    return NextResponse.json({ success: true, message: '2FA activado exitosamente' });
  } catch (error) {
