...
import { ChevronLeft, ChevronRight, Clock, User, X, Plus, Loader2, Edit2, Trash2, Phone, FileText, CheckCircle2 } from "lucide-react"
...
const STATUS_COLORS: Record<string, { bg: string; text: string; border: string; label: string }> = {
  scheduled: { bg: "bg-amber-50", text: "text-amber-800", border: "border-l-4 border-l-amber-400", label: "Programada" },
  confirmed: { bg: "bg-blue-50", text: "text-blue-800", border: "border-l-4 border-l-blue-500", label: "Confirmada" },
  completed: { bg: "bg-emerald-50", text: "text-emerald-800", border: "border-l-4 border-l-emerald-500", label: "Completada" },
  cancelled: { bg: "bg-red-50", text: "text-red-400 line-through", border: "border-l-4 border-l-red-300 opacity-60", label: "Cancelada" },
  no_show: { bg: "bg-gray-50", text: "text-gray-400", border: "border-l-4 border-l-gray-300", label: "No asistio" },
}
...
const START_HOUR = 8
const END_HOUR = 20
const HOURS = Array.from({ length: (END_HOUR - START_HOUR) * 2 + 1 }, (_, i) => START_HOUR * 60 + i * 30).filter(m => m <= END_HOUR * 60)
...
function CurrentTimeLine() {
  ...
  const top = ((minutes - START_HOUR * 60) / 30) * PX_PER_SLOT
...
export function CalendarGrid() {
  ...
  useEffect(() => {
    if (scrollRef.current && view !== "month") {
      ...
      const scrollTo = Math.max(0, ((mins - START_HOUR * 60) / 30) * PX_PER_SLOT - 120)
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          {view !== "month" && (
            <div className="hidden sm:flex items-center gap-2 text-xs text-gray-500">
              <span className="flex items-center gap-1"><span className="w-2 h-2 rounded-full bg-blue-500" /> {confirmedCount} confirmadas</span>
      ...
      {loading ? (
        ...
        <MonthView appointments={appointments} currentDate={currentDate} onSelect={setSelectedAppt} onDayClick={(d) => { setCurrentDate(d); setView("day") }} />
      ) : (
            ...
            {(view === "day" ? [currentDate] : weekDates).map((d, colIdx) => {
              ...
              return (
                <div key={colIdx} className={`relative border-l border-gray-50 ${isToday ? "bg-blue-50/20" : ""}`}>
                  ...
                  {HOURS.map(m => (
                    <div key={m}
                      className="h-10 border-b border-gray-50 cursor-pointer hover:bg-gray-50/50 transition"
                      onClick={() => {
                  ...
                  {dayAppts.map(appt => {
                    ...
                    const top = ((startMin - START_HOUR * 60) / 30) * PX_PER_SLOT
                    const height = Math.max(((endMin - startMin) / 30) * PX_PER_SLOT - 2, 20)
                    const colors = STATUS_COLORS[appt.status] || STATUS_COLORS.scheduled
                    return (
                      <div key={appt.id} onClick={() => setSelectedAppt(appt)}
                        className={`absolute left-1 right-1 rounded-r-lg ${colors.bg} ${colors.border} px-2 py-1 cursor-pointer hover:shadow-md transition-shadow overflow-hidden`}
                        style={{ top: `${top}px`, height: `${height}px` }}>
                        <p className={`text-[11px] font-semibold ${colors.text} truncate`}>{appt.patientName}</p>
                        {height > 30 && <p className="text-[10px] text-gray-500 truncate">{formatTime(appt.startTime)} &mdash; {appt.reason || "Consulta"}</p>}
                      </div>
                    )
                  })}
                </div>
        ...
        <Modal onClose={() => setSelectedAppt(null)} title="Detalle de Cita">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-semibold text-gray-900">{selectedAppt.patientName}</p>
                <div className="flex items-center gap-2 mt-0.5">
                  <a href={`https://wa.me/${selectedAppt.patientPhone}`} target="_blank" rel="noopener noreferrer"
                    className="inline-flex items-center gap-1 text-xs text-green-600 hover:text-green-700">
                    <Phone className="w-3 h-3" /> {selectedAppt.patientPhone}
                  </a>
                </div>
              </div>
            </div>
            ...
            <div className="bg-gray-50 rounded-lg p-3 space-y-2">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span>{selectedAppt.date} &mdash; {formatTime(selectedAppt.startTime)} a {formatTime(selectedAppt.endTime)}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <FileText className="w-4 h-4 text-gray-400" />
                <span>{selectedAppt.reason || "Consulta general"}</span>
              </div>
              <div className="flex items-center gap-2 text-sm">
            ...
            <div className="flex gap-2 pt-2 border-t border-gray-100">
              <button onClick={() => { setEditModal(selectedAppt); setSelectedAppt(null) }}
                className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-sm font-medium text-blue-700 bg-blue-50 rounded-lg hover:bg-blue-100 transition">
                <Edit2 className="w-3.5 h-3.5" /> Editar
              </button>
              {selectedAppt.status !== "completed" && selectedAppt.status !== "cancelled" && (
                <button onClick={() => handleComplete(selectedAppt.id)} disabled={saving}
                  className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2.5 text-sm font-medium text-emerald-700 bg-emerald-50 rounded-lg hover:bg-emerald-100 transition">
                  <CheckCircle2 className="w-3.5 h-3.5" /> Completar
                </button>
              )}
              {selectedAppt.status !== "cancelled" && (
                <button onClick={() => handleCancel(selectedAppt.id)} disabled={saving}
                  className="flex items-center justify-center gap-1.5 px-3 py-2.5 text-sm font-medium text-red-700 bg-red-50 rounded-lg hover:bg-red-100 transition">
          ...
          <div className="space-y-3">
            <label className="block"><span className="text-sm text-gray-600">Fecha</span>
              <input type="date" value={editModal.date} onChange={e => setEditModal({ ...editModal, date: e.target.value })}
                className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" /></label>
            ...
            <label className="block"><span className="text-sm text-gray-600">Estatus</span>
              <select value={editModal.status} onChange={e => setEditModal({ ...editModal, status: e.target.value })}
                className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between mb-4">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-gray-100 transition"><X className="w-4 h-4" /></button>
        </div>
        {children}
      </div>
    </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`inline-flex px-2 py-0.5 rounded-full text-xs font-medium ${colors.bg} ${colors.text.replace(" line-through", "")}`}>{colors.label}</span>
}
      ...
      <div className="space-y-3">
        <label className="block"><span className="text-sm text-gray-600">Nombre del paciente *</span>
          <input type="text" value={form.patientName} onChange={e => setForm({ ...form, patientName: e.target.value })}
            className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Juan Perez" /></label>
        <div className="grid grid-cols-2 gap-3">
          <label className="block"><span className="text-sm text-gray-600">Fecha</span>
        ...
        <label className="block"><span className="text-sm text-gray-600">Motivo</span>
          <input type="text" value={form.reason} onChange={e => setForm({ ...form, reason: e.target.value })}
            className="mt-1 w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:border-blue-500 focus:ring-1 focus:ring-blue-500 outline-none" placeholder="Consulta general" /></label>
...
function MonthView({ appointments, currentDate, onSelect, onDayClick }: {
  appointments: AppointmentData[]; currentDate: Date; onSelect: (a: AppointmentData) => void; onDayClick: (d: Date) => void
}) {
    ...
    <div className="p-4">
      <div className="grid grid-cols-7 gap-px bg-gray-100 rounded-xl overflow-hidden">
        ...
        {cells.map((day, i) => {
          ...
          const isToday = dateStr === todayStr
          return (
            <div key={i} className={`bg-white min-h-[80px] p-1.5 cursor-pointer hover:bg-gray-50/50 transition ${isToday ? "ring-2 ring-blue-200 ring-inset" : ""}`}
              ...
              {dayAppts.slice(0, 3).map(a => {
                ...
                return (
                  <button key={a.id} onClick={(e) => { e.stopPropagation(); onSelect(a) }}
                    className={`block w-full text-left text-[10px] px-1 py-0.5 rounded truncate mb-0.5 ${c.bg} ${c.text.replace(" line-through", "")}`}>
