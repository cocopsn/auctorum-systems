...
export default function SignupPage() {
  ...
  const inputCls = 'w-full px-3.5 py-2.5 rounded-lg border border-slate-700 bg-slate-800 text-white placeholder-slate-500 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors';
    ...
    <div className="min-h-screen bg-slate-950 flex flex-col justify-center items-center px-6 py-10">
      <div className="w-full max-w-md">
        <div className="text-center mb-8">
          ...
          <p className="text-sm text-slate-400 mt-1">Auctorum Systems — Plataforma de Gestión Inteligente</p>
        </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-2xl shadow-2xl p-8">
          {submitted ? (
            <div className="text-center">
              ...
              <h2 className="text-base font-semibold text-white mb-2">¡Cuenta creada!</h2>
              <p className="text-sm text-slate-400">
                Le hemos enviado un enlace de acceso a <span className="font-medium text-white">{email}</span>.
              </p>
              <p className="text-xs text-slate-500 mt-4">
                Su subdominio: <code className="font-mono text-slate-300">{slug}.auctorum.com.mx</code>
              </p>
            </div>
          ) : (
            <form onSubmit={onSubmit} noValidate className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-slate-300 mb-1.5">Su nombre completo</label>
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <span className="absolute right-3 top-1/2 -translate-y-1/2 text-xs text-slate-500 pointer-events-none">.auctorum.com.mx</span>
                </div>
              ...
              <button type="submit" disabled={!canSubmit}
                className="w-full bg-blue-600 hover:bg-blue-500 text-white font-medium py-3 px-4 rounded-lg text-sm disabled:opacity-50 disabled:cursor-not-allowed transition-colors inline-flex items-center justify-center gap-2">
                {submitting && <Loader2 className="w-4 h-4 animate-spin" />}
                Crear cuenta
              </button>
              <p className="text-xs text-slate-500 text-center pt-2">
                ¿Ya tiene cuenta? <Link href="/login" className="text-blue-400 hover:text-blue-300">Iniciar sesión</Link>
              </p>
            </form>
          )}
        </div>
