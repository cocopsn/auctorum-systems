...
export default async function PatientDetailPage({
  ...
  return (
    <div>
      <div className="mb-6">
        <a href="/dashboard/pacientes" className="text-sm text-gray-500 hover:text-gray-700">
          ← Volver a pacientes
        </a>
      </div>
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6 mb-6">
        <div className="flex items-start gap-4">
          ...
          <div className="text-right">
            ...
            <p className="text-2xl font-bold text-gray-900">${Number(patient.totalSpent ?? 0).toLocaleString()}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 mt-6 pt-4 border-t border-gray-100">
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900">{patient.totalAppointments}</p>
            <p className="text-xs text-gray-500">Citas</p>
          </div>
          <div className="text-center">
            <p className="text-2xl font-bold text-red-500">{patient.totalNoShows}</p>
          ...
          <div className="text-center">
            ...
            <p className="text-xs text-gray-500">Tasa no-show</p>
          </div>
        </div>
          ...
          <div className="mt-4 pt-4 border-t border-gray-100 space-y-2">
            {patient.allergies && (
              <div className="flex items-start gap-2 text-sm">
                ...
                <div><span className="font-medium">Alergias:</span> {patient.allergies}</div>
              </div>
            )}
            {patient.chronicConditions && (
              <div className="flex items-start gap-2 text-sm">
                <AlertTriangle className="w-4 h-4 text-orange-400 mt-0.5" />
                <div><span className="font-medium">Condiciones crónicas:</span> {patient.chronicConditions}</div>
              </div>
            )}
            {patient.insuranceProvider && (
              <div className="flex items-start gap-2 text-sm">
                <Shield className="w-4 h-4 text-blue-400 mt-0.5" />
                <div><span className="font-medium">Seguro:</span> {patient.insuranceProvider} {patient.insurancePolicy && `(${patient.insurancePolicy})`}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div className="space-y-2">
            {patientAppointments.map((appt) => (
              <div key={appt.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                <div>
                  <span className="text-sm font-medium text-gray-900">{appt.date}</span>
                  <span className="text-sm text-gray-400 ml-2">{appt.startTime.slice(0, 5)}</span>
                  {appt.reason && <span className="text-sm text-gray-500 ml-3">{appt.reason}</span>}
                </div>
          ...
          <div className="space-y-3">
            {notes.map((note) => (
              <div key={note.id} className="p-3 bg-gray-50 rounded-lg">
                ...
                {note.assessment && <p className="text-sm text-gray-700"><strong>Assessment:</strong> {note.assessment}</p>}
                {note.plan && <p className="text-sm text-gray-700"><strong>Plan:</strong> {note.plan}</p>}
              </div>
            ))}
          </div>
        </div>
